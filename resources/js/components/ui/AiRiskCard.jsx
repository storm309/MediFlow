        ...
        <div className={`card p-5 border ${sev.border} ${sev.bg}`}>
            ...
            {risk.risks?.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                    {risk.risks.map((r, i) => (
                        <span key={i} title={r.detail} className={`text-xs font-medium px-2 py-1 rounded-full capitalize cursor-help ${riskLevelColors[r.level] ?? 'bg-slate-100 text-slate-700'}`}>
            ...
            {/* Trend Prediction */}
            {risk.trend_prediction && (
                <div className={`flex items-center gap-2 px-3 py-2 rounded-lg mb-4 text-xs font-semibold ${
                    risk.trend_prediction === 'stable'        ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' :
                    risk.trend_prediction === 'moderate_risk' ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400' :
                                                                'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'
                }`}>
                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                    Health Trend: <span className="capitalize ml-1">{risk.trend_prediction.replace(/_/g, ' ')}</span>
                </div>
            )}

            {/* AI Explanation Box — WHY this score */}
            {risk.explanation_factors?.length > 0 && (
                <div className="mb-4 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                    <button
                        onClick={() => setShowExplanation(v => !v)}
                        className="w-full flex items-center justify-between px-3 py-2 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-xs font-semibold text-slate-600 dark:text-slate-400"
                    >
                        ...
                        <svg className={`w-3.5 h-3.5 transition-transform ${showExplanation ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    {showExplanation && (
                        <ul className="px-3 py-2 space-y-1.5 bg-white dark:bg-slate-900/40">
                            {risk.explanation_factors.map((factor, i) => (
